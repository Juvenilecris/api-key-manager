...
export default function MacApiKeyManager() {
  ...
  const [isLoadingData, setIsLoadingData] = useState(false); // 加载状态
  ...
  // --- API Helpers (Sync with Vercel KV) ---
  const fetchKeysFromCloud = async () => {
    setIsLoadingData(true);
    try {
      const res = await fetch('/api/storage', {
        method: 'GET',
        headers: { 'x-auth-token': '1006' } // 简单验证
      });
      if (res.ok) {
  ...
  useEffect(() => {
    ...
    if (session === 'nnwang-1006-session') {
      setIsLoggedIn(true);
      fetchKeysFromCloud(); // 登录成功后加载数据
    }
  }, [isLoggedIn]); // 依赖 isLoggedIn，确保登录后触发
  ...
  const saveKey = async () => {
    ...
    await saveKeysToCloud(updatedKeys); // 保存到云端
    setShowModal(false);
  };
  ...
  const deleteKey = async (id: string) => {
    ...
    await saveKeysToCloud(updated); // 保存到云端
  };
